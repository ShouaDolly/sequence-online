(() => {
  ...
  soundButton.onclick=()=>{muted=!muted;localStorage.setItem('sequence_sound_muted',muted?'1':'0');soundButton.textContent=muted?'🔇':'🔊';if(!muted)feedback('hint')};
  document.body.appendChild(soundButton);

  const hapticButton=document.createElement('button');
  hapticButton.type='button'; hapticButton.id='hapticToggle'; hapticButton.textContent=haptics?'📳':'📴'; hapticButton.title='Toggle haptic feedback';
  hapticButton.style.cssText='position:fixed;right:58px;bottom:12px;z-index:4000;width:40px;height:40px;border-radius:50%;border:1px solid #52749b;background:#0b2445;color:#fff;box-shadow:0 6px 18px #0006;cursor:pointer;font-size:18px;';
  hapticButton.onclick=()=>{haptics=!haptics;localStorage.setItem('sequence_haptics_enabled',haptics?'1':'0');hapticButton.textContent=haptics?'📳':'📴';if(haptics)buzz(18)};
  ...
  function markLockedSequences(playSound=false){
    ...
    if(playSound && newly)feedback('lock');
    return newly;
  }
  const boardObserver=new MutationObserver(()=>{ if(document.querySelector('#game:not(.hidden)')) markLockedSequences(true); });
  const start=()=>{const b=document.querySelector('#board');if(b){boardObserver.observe(b,{childList:true,subtree:true});markLockedSequences(false)}};
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start);else start();

  document.addEventListener('click',e=>{
    const t=e.target.closest?.('#cards .cardhand'); if(t)feedback('card');
    if(e.target.closest?.('#hintBtn'))feedback('hint');
    if(e.target.closest?.('#finishBtn'))feedback('finish');
    if(e.target.closest?.('#autoBtn'))feedback('auto');
    if(e.target.closest?.('#board .cell')){feedback('place');setTimeout(()=>markLockedSequences(false),40)}
  },true);
})();
